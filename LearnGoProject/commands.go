...
func OnEdit(s *discordgo.Session, editmsg *discordgo.MessageUpdate) {
	fmt.Println(editmsg.EditedTimestamp)
	if _, exists := m[editmsg.Author.Username]; exists {
		if editmsg.EditedTimestamp != ""{
			fmt.Println("m[editmsg.Author.Username]"+m[editmsg.Author.Username])
			fmt.Println("dm[m[editmsg.Author.Username]]"+dm[m[editmsg.Author.Username]])
